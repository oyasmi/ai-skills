...
本项目提供三个独立实现，共享相同的 SQLite 数据库 Schema：

| 目录 | 技术栈 | 目标平台 | 分发包体积 |
|---|---|---|---|
| `python/` | Python + CustomTkinter + APSW | 跨平台 | ~50MB (PyInstaller) |
...
三个实现共享相同的 SQLite 数据库 Schema，数据文件可以在不同实现间互通。
