...
5. TUI 要求实时进入终端时才使用 `attach`；headless 对话使用 `logs --follow`
6. `capture` 默认读取当前可观测输出；TUI 返回屏幕文本，结构化 harness 返回聚合内容
7. `logs` 读取结构化 harness 的完整可读 transcript，也能读取保留的 ended 实例
8. 使用 `summon` 时，要明确区分“新建”与“复用”
9. 一次性委派优先使用 `run`；需要保留新建会话时显式加 `--keep`
10. 每条首次任务指令应按需包含工作模式、目标、上下文、范围边界、完成定义和交付说明
11. 追加任务指令应基于 `inspect` 或 `capture` 的具体证据，只描述相对原任务的变化
12. 外部 Agent 的完成声明和自验不能替代编排 Agent 的直接验收
13. 长任务策略应集中写在“等待、输入与中断”章节，不要把同一规则在多个章节重复展开
14. 明确实例隔离不等于工作树隔离；多个写入者必须使用不同 worktree 和 `cwd`
