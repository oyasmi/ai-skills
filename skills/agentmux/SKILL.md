---
name: agentmux
description: 通过 `agentmux` CLI 委派和管理外部 AI coding agent：按角色选模板（planner/builder/reviewer/verifier/worker/documenter/scout 等）、创建或复用实例、编写首次与追加任务指令、等待和读取输出、纠偏、验证交付物以及停止实例。模板声明 model 与 effort（thinking level）档位，agentmux 翻译到 `claude-code-ndjson`、`codex-cli-execjson`、`pi-rpc` 等 headless 结构化 harness 以及基于 tmux 的 TUI harness。用户提到 `agentmux`、需要调用其他 CLI coding agent 或需要复用外部 Agent 完成任务时使用；明确需要时，也可用它设计或调整 agentmux 角色与团队模板。
---

# Agentmux

本 skill 主要面向 pipiclaw 这类上层个人助手：帮助它们选择合适角色、可靠委派任务并验收结果。多数任务只需调用一个最小足够角色；多角色团队模板是复杂任务中的进阶选项。
...
| 角色 | 典型场景 | 典型档位 |
| --- | --- | --- |
| `planner` | 需求澄清、方案设计、验收定义和任务拆解；可落盘需求/设计/实施文档 | 高档模型 + `high`/`max` |
| `builder` | 边界清楚的产品代码、单元测试和实现自查（默认入口） | 中高档模型 + `medium` |
| `reviewer` | 对方案、实现或文档独立挑错，可落盘评审报告 | 与产出者不同的模型家族 + `high`/`xhigh` |
| `verifier` | 问题复现、实际运行、冒烟/回归和运行数据分析 | 中低档模型 + `medium` |
| `documenter` | 文档、变更记录、commit 和最终交付 | 中档模型 + `medium` |
| `worker` | 数据对比、效果分析、批量转换和专项报告等通用工作 | 中档模型 + `medium` |
| `scout` | 便宜快速的单点事实查询 | 低档模型 + `low` |

`builder-hard` 等模板通常是某个职责的高强度档位，不是新的团队责任。如果只是某次任务更难，优先用 `--model`/`--effort` 临时升档；只在常驻工作方式也不同时才单独建模板。

选人原则：

1. **默认选最小足够角色**。一个边界清楚的实现只用 `builder`；一个事实查询只用 `scout`。不要为了形式完整而每次调用全部角色。
2. **需求未收敛时先用 `planner`**。让它交付方案、验收标准和任务契约；它可以落盘规划文档，不要用“规划角色必须只读”一类绝对规则阻止正常交付。
3. **评审与实证分开**。`reviewer` 通过独立阅读挑错，`verifier` 通过实际运行证明；静态审查不能代替冒烟或回归。
4. **审查尽量换模型家族**。让 `reviewer` 与产出者使用不同的模型家族，降低同源盲区。评审对象应是稳定文档、diff 或 commit。
5. **用 `verifier` 建立运行证据**。完成定义依赖真实运行、外部服务、性能或数据时，给它待验版本、环境、数据和编号验收标准。
6. **最终交付交给 `documenter`**。需要同步文档、变更记录、commit 和交付摘要时，把已批准方案、实现范围、评审结论和验证证据交给它。
7. **失败后先判断是否需要升档**。同一问题连续纠偏仍失败时，新建实例并用 `--model`/`--effort` 升档，同时带上已确认证据和已排除方向。
8. 只有"人要旁观、接管或调试终端"时才用可 `attach` 的 TUI 角色；它启动提示多、按键和状态推断都更脆弱。

需要设计、修改或理解多角色模板时，再读[七角色开发样例与模板参考](references/team-templates.md)。其中包含职责闭环、交接契约、改造方法，以及可复制的 [`development-team.yaml`](assets/development-team.yaml)。
...
1. **工作模式与目标**：明确要求事实查询、调查/规划、实现、评审、运行验证、文档/交付还是通用分析，并描述可观察的结果。
...
对复杂、模糊或高风险任务，先委派规划阶段，要求交付需求边界、实现方案、影响范围、假设、风险、验收标准和任务拆解；需要落盘时给出需求/设计/实施文档路径。审查方案后再把已批准结论交给实现角色。对边界清楚、易于验证的常规任务，在同一轮要求实现、测试和自查。除非执行过程本身是需求，否则描述结果和约束，不要过度规定具体步骤。
...
对高风险改动或长时间自主执行的任务，用 `reviewer` 角色的新实例独立审查稳定的方案、diff、commit 或文档——它尽量与产出者用不同的模型家族，降低同源盲区。向评审者提供原始完成标准和变更范围，要求报告具体位置、触发场景、影响和证据，不追逐纯风格偏好。

是否能共享 `cwd` 取决于实际写入行为，不取决于角色名称。如果 `planner`、`reviewer` 或 `verifier` 需要落盘文档、报告、脚本或运行产物，就把它们当作写入者：避免与其他实例在同一 checkout 并发修改重叠文件。只有模板使用了真正的只读 sandbox，且任务不需要落盘时，才可将 `cwd_shared` 视为预期提醒。
